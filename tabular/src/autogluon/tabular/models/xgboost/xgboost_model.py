...
class XGBoostModel(AbstractModel):
    ...
    def get_eval_metric(self, metric=None):
        if metric is None:
            metric = self.stopping_metric
        metric = get_metric(metric)
        eval_metric = xgboost_utils.convert_ag_metric_to_xgbm(ag_metric_name=metric.name, problem_type=self.problem_type)
        if eval_metric is None:
            eval_metric = xgboost_utils.func_generator(metric=metric, problem_type=self.problem_type)
        return eval_metric
    ...
    def _fit(self, X, y, X_val=None, y_val=None, time_limit=None, num_gpus=0, num_cpus=None, sample_weight=None, sample_weight_val=None, verbosity=2, **kwargs):
        ...
        if eval_set is not None and "callbacks" not in params:
            callbacks = []
            if generate_curves:
                callbacks.append(CustomMetricCallback(scorers, eval_set, self.problem_type, use_error=use_curve_metric_error))
        ...
        if generate_curves:
            ...
            train_curves = filter(eval_results["validation_0"], metric_names)
            val_curves = filter(eval_results['validation_1'], metric_names)
